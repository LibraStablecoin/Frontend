...
import Pool_USDC from 'constants/abi/Pool_USDC.json'
import Pool_DAI from 'constants/abi/Pool_DAI.json'
import LP from "constants/abi/LP.json";
...
import StableABI from 'constants/abi/Stable.json'
...
export function useMinterContract() {
  const { chainId } = useWeb3React()
  const address = useMemo(() => (chainId ? Pool[chainId] : undefined), [chainId])
  return useContract(address, Pool_USDC)
}
export function useMinterDAIContract() {
  const { chainId } = useWeb3React()
  const address = useMemo(() => (chainId ? PoolDAI[chainId] : undefined), [chainId])
  return useContract(address, Pool_DAI)
}
export function useLpaddr(){
  const { chainId } = useWeb3React()
  const address = useMemo(() => (chainId ? LPaddr[chainId] : undefined), [chainId])
  return useContract(address, LP)
}
export function useoracleaddr(){
  const { chainId } = useWeb3React()
  const address = useMemo(() => (chainId ? Oracle[chainId] : undefined), [chainId])
  return useContract(address, Oracleabi)
}
